...
dpo.fn <- function(dr, di) {
  result      <- dr * di
  print(paste("The value of dpo (ug) is", round(result,3), sep=" "))
  return(round(result, 3))
}
...
# Css
css.fn <- function(f, dpo, di, cl, wtkg) {
  numer  <- f * (dpo/di)
  denom  <- cl * wtkg
  result <- (numer/denom) 
...
ct.fn <- function (cmax, ke, time=0) {
        ...
        print(paste("The value of ct (ug/L) at t h is", round(result, 2), sep = " "))
        return(round(result, 2))
}
...
R0.fn <- function(css, cl) {
           result <- css * cl
           print(paste("The value of R0 (ug/hr) is", round(result, 2), sep = " "))
           return(round(result, 2))
}
####################### END ####################### 
